...
class EventsController extends Controller
{
    ...
    public function post_create(Request $request)
    {
        ...
        //Parametros recibidos del $request
        $title = $_POST['title'];
        ...
        //Conseguir el id del ultimo evento creado
        $table = DB::select("show table status like 'events'");
        if (!empty($table) && !is_null($table)) {
            $eventID = $table[0]->Auto_increment;
        }

        if (!empty($image)) {
            $filename = 'event_' . $eventID . '_' . $image->getClientOriginalName();
            $saveRoute = '/uploads/event/';
            Image::make($image)->resize(400,400)->save(public_path($saveRoute . $filename));
        }
        ...
        try {
            ...
            if (!empty($image)) {
                $eventDB->image = $this->getGlobalPath($saveRoute, $filename);
            }

            $eventDB->id_user = $user->id;
            $eventDB->save();
            if(is_array($array_id_group) || is_object($array_id_group)){
    ...
    public function post_update()//No se usa
    {
        $headers = getallheaders();
        $token = $headers['Authorization'];
        $key = $this->key;
    ...
    public function post_delete()
    {
        ...
        try {
            ...
            if ($eventBD->id_user == $user->id || $user->id_rol == 1) {
                $imageRoute = $eventBD->image;
                $foto = substr($imageRoute, strpos( $imageRoute, '/uploads'));
                $routeWorked = getcwd();
                unlink($routeWorked . $foto);
    ...
    public function get_commentsAndroid()
    {
        ...
        try {
            ...
            $commentsDB = Comments::where('id_event', $id_event)->get();
            if (count($commentsDB) == 0) {
                return $this->createResponse(400, "No existen comentarios");
            }
